...
fn verify_valid_proof() {
    ...
    assert!(Freda::verify_proof(&vk, &VALID_PROOF, &VALID_PUBS).is_ok());
}
...
fn return_the_same_bytes_as_public_inputs() {
    ...
    assert_eq!(Freda::pubs_bytes(&data).as_ref(), &data);
}

#[test]
fn validate_valid_vk() {
    let vk = freda_key();
    assert!(Freda::validate_vk(&vk).is_ok())
}

#[test]
fn reject_malformed_vk() {
    let mut vk = freda_key();

    *vk.mut_c0_x() = U256::zero();

    assert_eq!(
        Freda::validate_vk(&vk),
...
mod reject {
    ...
    fn invalid_pubs() {
        ...
        assert_eq!(
            Freda::verify_proof(&vk, &VALID_PROOF, &invalid_pubs),
    ...
    fn invalid_proof() {
        ...
        assert_eq!(
            Freda::verify_proof(&vk, &invalid_proof, &VALID_PUBS),
            Err(VerifyError::VerifyError)
        );
    }
    ...
    fn reject_malformed_proof() {
        ...
        assert_eq!(
            Freda::verify_proof(&vk, &malformed_proof, &VALID_PUBS),
