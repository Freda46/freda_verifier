...
impl TryInto<freda_verifier::VerificationKey> for Vk {
    type Error = ConvertError;

    fn try_into(self) -> Result<freda_verifier::VerificationKey, Self::Error> {
        Ok(freda_verifier::VerificationKey {
            power: self.power,
            k1: self.k1.into(),
            k2: self.k2.into(),
            w: self.w.into(),
...
impl Vk {
    pub fn try_into_freda_vk_unchecked(
        self,
    ) -> Result<freda_verifier::VerificationKey, ConvertError> {
        Ok(freda_verifier::VerificationKey {
            power: self.power,
            k1: self.k1.into(),
            k2: self.k2.into(),
...
mod test_utils {
    ...
    impl From<freda_verifier::VerificationKey> for Vk {
        fn from(value: freda_verifier::VerificationKey) -> Self {
            Self {
                power: value.power,
                k1: value.k1.into(),
                k2: value.k2.into(),
