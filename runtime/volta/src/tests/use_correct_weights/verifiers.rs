...
#[test]
fn pallet_freda_verifier() {
    use pallet_freda_verifier::Freda;
    let dummy_proof = [0; pallet_freda_verifier::PROOF_SIZE];
    let dummy_pubs = [0; pallet_freda_verifier::PUBS_SIZE];
    use pallet_freda_verifier::WeightInfo;

    assert_eq!(
        <<Runtime as pallet_verifiers::Config<Freda>>::WeightInfo as pallet_verifiers::WeightInfo<Freda>>::verify_proof(
            &dummy_proof,
            &dummy_pubs
        ),
        crate::weights::pallet_freda_verifier::ZKVWeight::<Runtime>::verify_proof()
    );
}
