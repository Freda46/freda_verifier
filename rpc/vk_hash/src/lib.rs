...
impl From<FredaVk> for pallet_freda_verifier::vk::Vk {
    fn from(vk: FredaVk) -> Self {
        Self {
            power: vk.power,
            k1: Fr(vk.k1),
            k2: Fr(vk.k2),
...
pub trait VKHashApi<ResponseType> {
    #[method(name = "freda")]
    fn freda(&self, vk: FredaVk) -> RpcResult<ResponseType>;
    #[method(name = "groth16")]
...
impl VKHashApiServer<H256> for VKHash {
    fn freda(&self, vk: FredaVk) -> RpcResult<H256> {
        Ok(Freda::vk_hash(&vk.into()))
    }
